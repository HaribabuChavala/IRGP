-- Application schema for the Instant Report Generation Platform
-- This file is executed by PostgreSQL init scripts when the container starts.
-- Keep these column types aligned with the SQLAlchemy models in api/models.py.
...
CREATE TABLE IF NOT EXISTS organizations (
    ...
    plan VARCHAR(32) NOT NULL DEFAULT 'free' CHECK (plan IN ('free', 'trial', 'year', 'half_year', 'starter', 'growth', 'enterprise')),
...
CREATE TABLE IF NOT EXISTS data_sources (
    ...
    name VARCHAR(255) NOT NULL,
    type VARCHAR(50) NOT NULL CHECK (type IN ('oracle', 'hive', 'postgres', 'mysql', 'excel', 'csv', 'api', 'warehouse')),
    host VARCHAR(255),
    port INTEGER,
    database_name VARCHAR(255),
    file_path TEXT,
    status VARCHAR(20) NOT NULL DEFAULT 'connected' CHECK (status IN ('connected', 'disconnected', 'error', 'pending')),
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_data_sources_org_name_ci
...
CREATE TABLE IF NOT EXISTS reports (
    id VARCHAR(36) PRIMARY KEY DEFAULT gen_random_uuid()::text,
    organization_id VARCHAR(36) NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    job_id VARCHAR(36) REFERENCES report_jobs(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    type VARCHAR(50) NOT NULL DEFAULT 'info' CHECK (type IN ('report', 'subscription', 'system', 'alert', 'info')),
...
CREATE TABLE IF NOT EXISTS reminders (
    ...
    title VARCHAR(255) NOT NULL,
    message TEXT NOT NULL,
    reminder_type VARCHAR(50) NOT NULL DEFAULT 'subscription' CHECK (reminder_type IN ('subscription', 'renewal', 'payment', 'system')),
    due_at TIMESTAMPTZ,
